...
const PRODUCTS = [
  {
    id: 1,
    name: "WORD IS BOND TEE",
    category: "TOPS",
    price: 68,
    tag: "NEW DROP",
    desc: "Heavyweight 340gsm cotton. Oversized cut. Screen-printed verse on chest.",
    sizes: ["XS", "S", "M", "L", "XL", "XXL"],
    colors: ["#f5f0e8", "#1a1a1a", "#0d6e5a"],
    image: null,
  },
  {
    id: 2,
    ...
    price: 148,
    tag: "BESTSELLER",
    desc: "Waxed cotton utility cargo. Embroidered cross detail at thigh pocket.",
    sizes: ["S", "M", "L", "XL"],
    colors: ["#2c2418", "#1a1a1a"],
    image: null,
  },
  {
    id: 3,
    name: "GRACE PERIOD HOODIE",
    category: "TOPS",
    price: 128,
    tag: "LIMITED",
    desc: "500gsm French terry. Dropped shoulders. Faith script across the back.",
    sizes: ["XS", "S", "M", "L", "XL", "XXL"],
    colors: ["#f5f0e8", "#0d6e5a", "#1a1a1a"],
    image: null,
  },
  {
    id: 4,
    name: "EXODUS WINDBREAKER",
    category: "OUTERWEAR",
    price: 218,
    tag: "COLLAB",
    desc: "Technical ripstop shell. Quilted lining. Zipper pull engraved with cross.",
    sizes: ["S", "M", "L", "XL"],
    colors: ["#0d1f1a", "#1a1a1a", "#f5f0e8"],
    image: null,
  },
  {
    id: 5,
    ...
    price: 48,
    tag: null,
    desc: "6-panel structured cap. Embroidered arch logo. Adjustable strap.",
    sizes: ["ONE SIZE"],
    colors: ["#1a1a1a", "#f5f0e8", "#0d6e5a"],
    image: null,
  },
  {
    id: 6,
    ...
    price: 88,
    tag: "NEW DROP",
    desc: "Heavy twill. Relaxed fit. Hidden pocket with lamb's wool lining.",
    sizes: ["S", "M", "L", "XL", "XXL"],
    colors: ["#2c2418", "#f5f0e8"],
    image: null,
  },
];
...
const TEAL = "#0d9e75";
const TEAL_DARK = "#0a7a5a";

const ProductCard = ({ product, onOpen, onWishlist, wishlisted, onAddCart }) => (
  <div className="group cursor-pointer flex flex-col" style={{ borderTop: "2px solid #1a1a1a" }}>
    <div
      className="relative overflow-hidden flex items-center justify-center"
      style={{
        background: product.id % 2 === 0 ? "#1a1a1a" : "#f5f0e8",
        aspectRatio: "3/4",
        minHeight: 260,
      }}
    ...
    >
      ...
      {product.tag && (
        <span
          className="absolute top-3 left-3 text-white text-xs font-bold px-2 py-1"
    ...
    <div className="pt-3 pb-4 flex flex-col gap-1" onClick={() => onOpen(product)}>
      <div className="flex justify-between items-start">
        <h3 style={{ fontFamily: "'Anton', sans-serif", fontSize: 15, letterSpacing: 1.5, lineHeight: 1.2, color: "#1a1a1a" }}>
          {product.name}
        </h3>
        <span style={{ fontFamily: "'Anton', sans-serif", fontSize: 15, color: TEAL }}>
          ${product.price}
        </span>
      </div>
    ...
    <div className="fixed inset-0 z-50 flex justify-end" style={{ background: "rgba(0,0,0,0.55)" }} onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-6 py-5" style={{ borderBottom: "2px solid #1a1a1a" }}>
          ...
          <button onClick={onClose} style={{ fontFamily: "'Anton', sans-serif", fontSize: 12, letterSpacing: 2, color: TEAL }}>CLOSE ✕</button>
        </div>
        ...
        {cart.length === 0 ? (
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-4">
            {cart.map((item) => (
              <div key={item.id + item.selectedSize} className="flex gap-4" style={{ borderBottom: "1px solid #eee", paddingBottom: 16 }}>
                ...
                <div className="flex-1 flex flex-col gap-1">
                  ...
                  <span style={{ fontSize: 11, color: "#999", letterSpacing: 1 }}>SIZE: {item.selectedSize || "M"}</span>
                  <div className="flex items-center gap-2 mt-1">
                    <button onClick={() => onQty(item.id, item.selectedSize, -1)} style={{ width: 24, height: 24, border: "1px solid #ddd", background: "none", fontWeight: 700 }}>−</button>
                    <span style={{ fontSize: 13, fontWeight: 700, minWidth: 20, textAlign: "center" }}>{item.qty}</span>
                    <button onClick={() => onQty(item.id, item.selectedSize, 1)} style={{ width: 24, height: 24, border: "1px solid #ddd", background: "none", fontWeight: 700 }}>+</button>
                    <button onClick={() => onRemove(item.id, item.selectedSize)} style={{ marginLeft: "auto", fontSize: 10, color: "#aaa", letterSpacing: 1, fontFamily: "'Anton', sans-serif", background: "none", border: "none" }}>REMOVE</button>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="px-6 py-5" style={{ borderTop: "2px solid #1a1a1a" }}>
          ...
          <button
            className="w-full py-4 text-white font-bold tracking-widest"
            style={{ background: "#1a1a1a", fontFamily: "'Anton', sans-serif", letterSpacing: 4, fontSize: 14, border: `2px solid ${TEAL}` }}
          >
            CHECKOUT — ${total}
          </button>
...
const WishlistDrawer = ({ wishlist, onClose, onRemove, onAddCart }) => (
  <div className="fixed inset-0 z-50 flex justify-end" style={{ background: "rgba(0,0,0,0.55)" }} onClick={onClose}>
    <div
      className="h-full flex flex-col"
      style={{ width: "min(420px, 100vw)", background: "#fff", borderLeft: `4px solid ${TEAL}` }}
              ...
              <div className="flex-1 flex flex-col gap-1">
                ...
                <div className="flex gap-2 mt-2">
                  ...
                  <button
                    ...
                    style={{ fontSize: 10, color: "#aaa", letterSpacing: 1, fontFamily: "'Anton', sans-serif", background: "none", border: "none" }}
                  >REMOVE</button>
                </div>
              </div>
            </div>
...
const ProductModal = ({ product, onClose, onAddCart, onWishlist, wishlisted }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center" style={{ background: "rgba(0,0,0,0.75)" }} onClick={onClose}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 md:px-8 py-6 md:py-8 flex flex-col gap-5">
          <button onClick={onClose} style={{ alignSelf: "flex-end", fontFamily: "'Anton', sans-serif", fontSize: 11, letterSpacing: 2, color: "#aaa", background: "none", border: "none" }}>CLOSE ✕</button>

          <div>
            {product.tag && (
              <span className="text-white px-2 py-1 text-xs mb-2 inline-block" style={{ background: TEAL, fontFamily: "'Anton', sans-serif", letterSpacing: 2, fontSize: 10 }}>{product.tag}</span>
            )}
              ...
              {product.colors.map((c, i) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: "50%",
                    background: c,
                    border: selectedColor === i ? `3px solid ${TEAL}` : "2px solid #ddd",
                  }}
                />
              ))}
            </div>
              ...
              {product.sizes.map((s) => (
                <button
                  ...
                  style={{
                    ...
                    fontSize: 12,
                    letterSpacing: 2,
                    border: selectedSize === s ? `2px solid ${TEAL}` : "1px solid #ddd",
                    color: selectedSize === s ? TEAL : "#1a1a1a",
                    background: "none",
                  }}
                >{s}</button>
              ))}
          ...
          <div className="flex gap-3 mt-2">
            <button
              onClick={() => { onAddCart({ ...product, selectedSize: selectedSize || product.sizes[0] }); onClose(); }}
              className="flex-1 py-4 text-white font-bold"
              style={{ background: "#1a1a1a", fontFamily: "'Anton', sans-serif", letterSpacing: 4, fontSize: 13 }}
            >ADD TO CART</button>
            <button
              onClick={() => onWishlist(product)}
              style={{
                width: 52,
                border: `2px solid ${wishlisted ? TEAL : "#ddd"}`,
                background: wishlisted ? TEAL : "none",
                display: "flex",
                alignItems: "center",
...
export default function App() {
  ...
  const addToCart = (product) => {
    setCart((prev) => {
      const key = `${product.id}_${product.selectedSize || "M"}`;
      ...
      return [...prev, { ...product, qty: 1, selectedSize: product.selectedSize || product.sizes[0] }];
    });
    setCartOpen(true);
  };

  const removeFromCart = (id, size) => setCart((p) => p.filter((i) => !(i.id === id && i.selectedSize === size)));
  const adjustQty = (id, size, delta) => setCart((p) => p.map((i) => i.id === id && i.selectedSize === size ? { ...i, qty: Math.max(1, i.qty + delta) } : i));
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-link { font-family: 'Anton', sans-serif; font-size: 12px; letter-spacing: 3px; color: #1a1a1a; text-decoration: none; cursor: pointer; transition: color 0.2s; }
      ...
      <div className="ticker-wrap">
        <span className="ticker" style={{ fontFamily: "'Anton', sans-serif", fontSize: 11, letterSpacing: 4, color: "#fff" }}>
          {Array(8).fill("✦ NEW SEASON DROP ✦ FREE SHIPPING $200+ ✦ FAITH OVER FEAR ✦ REMNANT SS25 ✦").join("  ")}
        </span>
      </div>
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 20px" }}>
          <div className="flex items-center justify-between" style={{ height: 60 }}>
            <div className="flex items-center gap-8">
              <button onClick={() => { setPage("home"); setMobileMenu(false); }} style={{ fontFamily: "'Anton', sans-serif", fontSize: 22, letterSpacing: 3, color: "#1a1a1a", background: "none", border: "none", cursor: "pointer" }}>
                REMNANT
              </button>
              ...
              <div className="flex flex-col md:flex-row gap-6 items-start md:items-end justify-between mt-4">
                ...
                <div className="flex gap-4">
                  ...
                  <button
                    ...
                    style={{ border: `2px solid #1a1a1a`, fontFamily: "'Anton', sans-serif", letterSpacing: 4, fontSize: 13, background: "none" }}
                  >SS25 LOOKBOOK</button>
                </div>
              </div>
            </div>
          </section>
            ...
            <div style={{ maxWidth: 1280, margin: "0 auto" }}>
              <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                ...
                <p style={{ fontFamily: "'Playfair Display', serif", fontStyle: "italic", fontSize: "clamp(16px, 2vw, 22px)", color: "rgba(255,255,255,0.85)", maxWidth: 400, lineHeight: 1.7 }}>
                  "For we are God's handiwork, created in Christ Jesus to do good works." — Every piece tells a story. Every stitch a statement.
                </p>
              </div>
            </div>
          </section>
          ...
          <footer style={{ background: "#1a1a1a", padding: "48px 20px 24px" }}>
            <div style={{ maxWidth: 1280, margin: "0 auto" }}>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-12">
                <div>
                  <p style={{ fontFamily: "'Anton', sans-serif", fontSize: 24, letterSpacing: 3, color: "#fff", marginBottom: 8 }}>REMNANT</p>
                ...
                {[["SHOP", ["All Products", "New Arrivals", "SS25 Collection", "Sale"]], ["INFO", ["About Us", "Sustainability", "Size Guide", "Contact"]], ["LEGAL", ["Privacy Policy", "Terms of Service", "Returns", "Shipping"]]].map(([title, links]) => (
                  <div key={title}>
              ...
              <div style={{ borderTop: "1px solid #333", paddingTop: 20, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                <p className="mono" style={{ color: "#555" }}>© 2025 REMNANT. ALL RIGHTS RESERVED.</p>
            ...
            {CATEGORIES.map((cat) => (
              <button
                ...
                style={{
                  ...
                  letterSpacing: 3,
                  padding: "10px 16px",
                  background: activeCategory === cat ? "#1a1a1a" : "none",
                  color: activeCategory === cat ? "#fff" : "#888",
                  border: "none",
                  cursor: "pointer",
                  whiteSpace: "nowrap",
                  borderBottom: activeCategory === cat ? `3px solid ${TEAL}` : "3px solid transparent",
                  transition: "all 0.2s",
                }}
              >{cat}</button>
            ))}
      ...
      {selectedProduct && (
        <ProductModal
          ...
          onAddCart={addToCart}
          onWishlist={toggleWishlist}
          wishlisted={wishlist.some((w) => w.id === selectedProduct.id)}
        />
      )}
